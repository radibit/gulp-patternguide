...
var
  ...
  getItemData = function (filePath, isDir) {
    ...
    currentPath = filePath.replace(patternsOptions.views, '');

    if (!isDir && sgUtil.getFileExtension(currentPath) === patternsOptions.sourceExt) {
      sourcePath = currentPath;
      currentPath = sgUtil.replaceFileExtension(currentPath, patternsOptions.targetExt);
    }
  ...
  collectPatternsRecursively = function (filePath) {
    ...
    if (currentItemData.isDir) {
      ...
      if (sgUtil.getFileExtension(currentItemData.name) !== patternsOptions.targetExt) {
        return;
      }
  ...
  readPatternStructure = function () {
    ...
    return readPatternStructureRecursively(patternsOptions.views);
  },

  collectPatternFiles = function () {
    collectedPatterns = {};
    collectPatternsRecursively(patternsOptions.views);
